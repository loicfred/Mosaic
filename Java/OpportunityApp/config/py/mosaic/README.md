...
Use the same Olist CSV copies as the committed models. For the two sales-model inputs,
`Get-FileHash datasets/olist_orders_dataset.csv -Algorithm SHA256` must return
`8DF58EF3D2D7E9944010F7BEECD9B75367F5588EC6E3C91CEC19AE3345EF9ECF`, and the
same command for `olist_order_items_dataset.csv` must return
`0BC4D068C4FE38CBB01BD90E8746E3C613FE7B4BAEF75FAB7B0E329701C3E279`.
Each model's JSON metadata records the exact hashes of its own input files. If a teammate's
copy differs, obtain the agreed CSV copy before running the demo; changing line endings
also changes a hash.
...
```powershell
cd Java/OpportunityApp/config/py/mosaic
...
| `GET /api/sales/forecast?horizon=3` | 1–6 month sales forecast using the lowest-error backtest method, with interval, baselines and backtest metrics | sales_forecast |
...
it while sales fell ~13% in June 2018 and stayed flat. The API and the investment suggestion
therefore use the last-month rule for the displayed forecast; the model remains in the
backtest table for comparison. The rule's displayed range uses its six one-step backtest
errors and is only an indication of uncertainty.
