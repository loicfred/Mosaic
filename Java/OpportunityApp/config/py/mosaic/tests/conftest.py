...
Per month: order ``<month>-a`` (seller s1 in SP, two items 100 + 50, on time, review 5, one card payment
in 1 instalment, a new customer each month) and order ``<month>-b`` (seller s2 in RJ, one item 200, late in
even months with review 1, otherwise on time with review 4, paid by card in 3 instalments plus a voucher, always
the same returning customer). Monthly sales are therefore always 350.0.
"""
import csv
...
from app.config import (
    CASHFLOW_FILE, CUSTOMERS_FILE, GEOLOCATION_FILE, ITEMS_FILE, ORDERS_FILE, PAYMENTS_FILE, PRODUCTS_FILE,
...
COLUMNS = {
    ...
    PAYMENTS_FILE: ["order_id", "payment_sequential", "payment_type", "payment_installments", "payment_value"],
    PRODUCTS_FILE: [
...
def _customer(order_id, zip_prefix, state, unique_id=None):
    return {
        "customer_id": f"c-{order_id}", "customer_unique_id": unique_id or f"u-{order_id}",
        "customer_zip_code_prefix": zip_prefix, "customer_city": "x", "customer_state": state,
    }


def _payment(order_id, sequential, payment_type, installments, value):
    return {"order_id": order_id, "payment_sequential": sequential, "payment_type": payment_type,
...
def write_fixture_csvs(directory: Path, drop_month: str | None = None, include_cashflow: bool = True) -> None:
    ...
    for month in FIXTURE_MONTHS:
        ...
        payments.append(_payment(a, 1, "credit_card", 1, 170.0))
        ...
        customers.append(_customer(b, "20000", "RJ", RETURNING_CUSTOMER))
        payments += [_payment(b, 1, "credit_card", 3, 150.0), _payment(b, 2, "voucher", 1, 60.0)]
    ...
    for order_id, status in (("cancelled-1", "canceled"), ("unavailable-1", "unavailable")):
        ...
        payments.append(_payment(order_id, 1, "boleto", 1, 1009.0))
    ...
    tables = {
        ORDERS_FILE: orders, ITEMS_FILE: items, PAYMENTS_FILE: payments, PRODUCTS_FILE: PRODUCTS, SELLERS_FILE: SELLERS,
