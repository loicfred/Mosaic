...
def test_late_variants_and_low_review_split(fixture_state):
    ...
    # rates held: a 0% change reproduces the baseline's low reviews, not a figure from another period
    assert held["expected_low_reviews_per_month"] == pytest.approx(result["baseline"]["monthly_low_reviews"])


def test_low_reviews_scale_with_orders_when_rates_hold(fixture_state):
    monthly, frame = fixture_state
    fall = impact.compute_sales_impact(monthly, frame, horizon=1, sales_change_pct=-10.0)
    held = fall["consequences"]["late"]["rate_held"]
    assert held["expected_low_reviews_per_month"] == pytest.approx(fall["baseline"]["monthly_low_reviews"] * 0.9)


def test_fitted_variant_is_dropped_when_its_line_misses_the_recent_rate():
    evidence = {"recent_late_rate": 0.036, "p_low_given_late": 0.5, "p_low_given_on_time": 0.1, "aov": 100.0}
    missing = {"slope": 0.0000112, "intercept": 0.0048}  # 7.5% at 6,266 orders against 3.6% observed
    assert impact._fitted_late_outcome(dict(missing), 5640.0, 6266.0, 1, evidence) is None
    close = {"slope": 0.0000050, "intercept": 0.0047}  # 3.6% at 6,266 orders
    assert impact._fitted_late_outcome(dict(close), 5640.0, 6266.0, 1, evidence)["late_rate"] == pytest.approx(0.0329, abs=1e-4)
