...
def test_fit_evaluate_and_predict(frame):
    ...
    unseen = frame.head(3).assign(category="never_seen_before", seller_state="ZZ")
    assert risk.predict_risk(model, spec, unseen).shape == (3,)
    single_class = risk.evaluate(model, spec, test[test["late"] == 0])
    assert single_class["roc_auc"] is None and single_class["average_precision"] is None
...
def test_train_all_writes_both_artifacts(fixture_dir, tmp_path):
    ...
    for name, meta in metadata.items():
        ...
        assert len(meta["dataset_hashes"]) == 9  # every Olist file the API reads, payments included
