...
@router.get("/forecast", summary="Sales forecast using the method with the lowest rolling backtest error")
...
def serialise_forecast(monthly: MonthlySales, model, metadata: dict, horizon: int) -> dict:
    ...
    scores = metadata["evaluation"][method]
    residual_std = metadata["residual_std"] if method == "model" else pstdev(
        point["actual"] - point["predicted"] for point in scores["points"]
    )

    forecast = []
    for label, point in zip(labels, sales_forecasting.forecast_by_method(method, model, values, horizon)):
        lower, upper = sales_forecasting.prediction_interval(point, residual_std)
    ...
    return {
        ...
        "limitations": metadata["limitations"] + ([
            "The selected simple rule's range uses its six one-step backtest errors; it is only an indication of uncertainty."
        ] if method != "model" else []),
    }
