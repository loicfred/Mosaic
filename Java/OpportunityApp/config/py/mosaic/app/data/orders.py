...
from app.data.loaders import (
    ...
    load_orders_full, load_payments, load_products, load_reviews, load_sellers,
)
from app.config import EXCLUDED_STATUSES
...
def build_order_features(datasets_dir: Path) -> OrderFeatures:
    ...
    frame = frame.merge(_aggregate_payments(load_payments(datasets_dir)), on="order_id", how="left")
    frame["cancelled"] = frame["order_status"].isin(EXCLUDED_STATUSES).astype(float)
    ...
    frame = _add_customer_history(frame)
    ...
    exclusions = {
        ...
        "orders_without_payment": int(frame["payment_total"].isna().sum()),
    }
...
def _aggregate_payments(payments: pd.DataFrame) -> pd.DataFrame:
    """One row per order: total paid, the most instalments of any payment, and the type that paid the most."""
    ...
    return totals.join(main).reset_index()
...
def _add_time_features(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["purchase_period"] = purchase.dt.strftime("%Y-%m")
...
def _add_customer_history(frame: pd.DataFrame) -> pd.DataFrame:
    """Whether the same person (customer_unique_id) placed an earlier order; Olist gives each order its own customer_id."""
    frame = frame.sort_values(["purchase_ts", "order_id"], kind="stable")
    known = frame["customer_unique_id"].notna()
    prior = frame.loc[known].groupby("customer_unique_id").cumcount()
    frame["returning_customer"] = np.where(known, (prior.reindex(frame.index) > 0).astype(float), np.nan)
    return frame
