...
cause. The "rate holds" variant is always returned; the "rate moves with volume" variant only
when the fitted line reproduces the recent late rate, so it cannot contradict the baseline.
"""
...
from app.analysis.populations import labelled_orders_in_range, purchase_month
...
def seller_capacity_strain(
    ...
    baseline_months: list[str] | None = None,
) -> dict:
    ...
    recent = set(baseline_months if baseline_months is not None else months[-recent_months:])
...
def compute_sales_impact(
    ...
    available_categories = sorted(orders_frame["category"].dropna().unique().tolist()) if "category" in orders_frame else []
    if category is not None and category not in available_categories:
        raise ValueError("Choose an available category or the whole business.")
    if "order_status" in orders_frame:
        orders_frame = orders_frame[~orders_frame["order_status"].isin(EXCLUDED_STATUSES)]
    if category is not None:
        orders_frame = orders_frame[orders_frame["category"] == category].copy()
        orders_frame["month"] = purchase_month(orders_frame)
        selected = orders_frame.groupby("month").agg(orders=("total_price", "size"), sales=("total_price", "sum"))
        months = selected.reindex(months["month"], fill_value=0).rename_axis("month").reset_index()
    ...
    scenario = {"horizon": horizon, "sales_change_pct": sales_change_pct, "recent_months": recent_months, "category": category,
                "population": "Orders assigned to the category of their highest-priced item; whole order sales are included."}
    baseline_block = {
    ...
    if baseline_orders <= 0 or baseline_sales <= 0:
        return {
            "available_categories": available_categories, "scenario": scenario, "baseline": baseline_block, "consequences": None,
    ...
    projected_sales = _money(Decimal(str(baseline_sales)) * Decimal(str(growth_factor)))
    projected_orders = baseline_orders * growth_factor
    recent_frame = orders_frame[purchase_month(orders_frame).isin(recent["month"])] if len(orders_frame) else orders_frame
    month_count = len(recent)
    baseline_block["monthly_late_orders"] = evidence["recent_late_orders"] / month_count if evidence["recent_delivered_orders"] else None
    reviews = recent_frame["low_review"].dropna() if "low_review" in recent_frame else pd.Series(dtype=float)
    baseline_block["monthly_low_reviews"] = float(reviews.sum()) / month_count if len(reviews) else None
    baseline_block["sellers_at_capacity"] = seller_capacity_strain(orders_frame, 1.0, recent_months, data_range=data_range, baseline_months=list(recent["month"]))
    evidence["recent_reviewed_orders"] = int(len(reviews))
    evidence["recent_low_review_rate"] = float(reviews.mean()) if len(reviews) else None
    low_per_order = baseline_block["monthly_low_reviews"] / baseline_orders if baseline_block["monthly_low_reviews"] is not None else None
    evidence["recent_low_reviews_per_order"] = low_per_order
    ...
    consequences = {
        ...
        "horizon_sales": _money(Decimal(str(projected_sales)) * horizon),
        ...
        "late": {
            "rate_held": _late_outcome(evidence["recent_late_rate"], projected_orders, horizon, evidence, "recent_3_month_rate",
                                       low_per_order),
            "rate_fitted": _fitted_late_outcome(evidence["volume_late_fit"], projected_orders, baseline_orders, horizon, evidence),
        },
        "sellers_at_capacity": seller_capacity_strain(orders_frame, growth_factor, recent_months, data_range=data_range, baseline_months=list(recent["month"])),
    }
    return {
        "available_categories": available_categories, "scenario": scenario, "baseline": baseline_block, "consequences": consequences,
...
def _seller_month_counts(orders_frame: pd.DataFrame, data_range: tuple[str, str]) -> pd.DataFrame:
    ...
    frame["month"] = purchase_month(frame)
...
def _observed_rates(orders_frame, recent, data_range) -> dict:
    ...
    reviewed = recent_labelled[recent_labelled["low_review"].notna()]

    by_month = labelled.groupby("month").agg(orders=("late", "size"), late_rate=("late", "mean"))
    return {
        "aov": float(recent["sales"].sum() / recent["orders"].sum()),
        "recent_late_rate": float(recent_labelled["late"].mean()) if len(recent_labelled) else None,
...
def _conditional_low_review(reviewed: pd.DataFrame, late: int) -> float | None:
    subset = reviewed[reviewed["late"] == late]
    return float(subset["low_review"].mean()) if len(subset) else None


def _late_outcome(late_rate: float | None, projected_orders: float, horizon: int, evidence: dict, basis: str,
                  low_per_order: float | None = None) -> dict | None:
    ...
    parts = [(expected_late, evidence["p_low_given_late"]), (on_time, evidence["p_low_given_on_time"])]
    if low_per_order is not None:
        expected_low = projected_orders * low_per_order
    else:
        expected_low = None if any(weight > 0 and rate is None for weight, rate in parts) else sum(weight * rate for weight, rate in parts if rate is not None)
    return {
        "basis": basis,
        "late_rate": late_rate,
        ...
        "expected_low_reviews_over_horizon": expected_low * horizon if expected_low is not None else None,
        "sales_exposed_per_month": expected_late * evidence["aov"],
        "sales_exposed_over_horizon": expected_late * evidence["aov"] * horizon,
    }


def _fitted_late_outcome(fit: dict | None, projected_orders: float, baseline_orders: float, horizon: int,
                         evidence: dict) -> dict | None:
    if fit is None:
        return None
    at_baseline = fit["intercept"] + fit["slope"] * baseline_orders
    recent = evidence["recent_late_rate"]
    fit["baseline_gap_pp"] = (at_baseline - recent) * 100 if recent is not None else None
    if recent is None or abs(at_baseline - recent) > FIT_TOLERANCE:
        return None
...
def _money(value) -> float:
    """Round hypothetical BRL amounts to cents with decimal arithmetic."""
