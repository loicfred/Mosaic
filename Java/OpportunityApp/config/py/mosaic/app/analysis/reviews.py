...
def monthly_low_review_rate(frame: pd.DataFrame, data_range: tuple[str, str] = DATA_RANGE) -> list[dict]:
    return monthly_rate(frame, "low_review", "reviewed", "low", data_range)
...
def score_unreviewed(frame: pd.DataFrame, model, limit: int) -> list[dict]:
    ...
    candidates = frame[(frame["delivered"] == 1) & frame["late"].notna() & frame["review_score"].isna()]
    if candidates.empty:
        return []
    return records(risk.rank(model, "low_review", candidates, limit)[ORDER_ID_FIELDS + ["risk"]])
