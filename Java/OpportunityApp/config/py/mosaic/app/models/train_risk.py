...
Run from ``Java/OpportunityApp/config/py/mosaic``: ``python -m app.models.train_risk``
"""
...
from app.models import risk, save_artifact
...
def train_all(
    ...
    for name, spec in risk.MODEL_SPECS.items():
        ...
        save_artifact(models_dir, name, model, meta)
        metadata[name] = meta
    return metadata
