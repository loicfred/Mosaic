...
Run from ``AI/``: ``python -m app.forecast.train``
"""
import json
...
ARTIFACT_NAME = "sales_forecast.joblib"
METADATA_NAME = "sales_forecast.json"
LIMITATIONS = [
...
def train(datasets_dir: Path = DATASETS_DIR, models_dir: Path = MODELS_DIR) -> dict:
    ...
    (models_dir / METADATA_NAME).write_text(json.dumps(metadata, indent=2), encoding="utf-8")
    return metadata
