...
from app.config import ALL_DATASET_FILES, CASHFLOW_FILE, CORS_ORIGINS, DATASETS_DIR, MODELS_DIR
...
def create_app(datasets_dir: Path = DATASETS_DIR, models_dir: Path = MODELS_DIR, auto_train: bool = False) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        cashflow_file_present = (datasets_dir / CASHFLOW_FILE).exists()
        app.state.cashflow = load_cashflow(datasets_dir) if cashflow_file_present else None
        app.state.datasets_dir = datasets_dir
        app.state.dataset_hashes = dict(dataset_hashes(datasets_dir, ALL_DATASET_FILES))
        if cashflow_file_present:
            app.state.dataset_hashes.update(dataset_hashes(datasets_dir, (CASHFLOW_FILE,)))
        if auto_train:
