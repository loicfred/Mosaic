...
- Added the entity `SmallBusinessCashflow` (`Java/OpportunityApp/src/main/java/mu/mosaic/opportunity/obj/entity/SmallBusinessCashflow.java`, table `small_business_cashflow`), with columns named as in `small_business_cashflow.csv`. As the user decided, it only describes the file: it is not in `BusinessDatabase.TABLES`, so nothing imports or exports it, and the Python API keeps reading the CSV directly. `mvnw compile` passes.
