...
ORDER_ID_FIELDS = ["order_id", "purchase_ts", "order_delivered_customer_date", "seller_id", "seller_state",
                   "customer_state", "category", "total_price", "delivery_days", "days_late"]


def _reviewed_in_range(frame: pd.DataFrame, data_range: tuple[str, str]) -> pd.DataFrame:
    month = frame["purchase_ts"].dt.strftime("%Y-%m")
    start, end = data_range
    return frame[frame["low_review"].notna() & month.between(start, end)].assign(month=month)


def monthly_low_review_rate(frame: pd.DataFrame, data_range: tuple[str, str] = DATA_RANGE) -> list[dict]:
    reviewed = _reviewed_in_range(frame, data_range)
    monthly = reviewed.groupby("month").agg(reviewed=("low_review", "size"), low=("low_review", "sum")).reset_index()
...
def low_review_by_lateness(frame: pd.DataFrame, data_range: tuple[str, str] = DATA_RANGE) -> dict:
    ...
    for name, mask in (("late", reviewed["late"] == 1), ("on_time", reviewed["late"] == 0)):
        ...
        result[name] = {"reviewed": int(len(subset)), "low": low, "low_rate": low / len(subset) if len(subset) else None}
    return result
