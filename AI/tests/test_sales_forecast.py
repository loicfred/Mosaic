...
def test_baselines_are_constant_over_horizon():
    assert sf.naive_last([1, 2, 3], 2) == [3, 3]
    assert sf.mean_last_3([1, 2, 3, 4], 2) == [3, 3]
...
def test_error_metrics_with_only_zero_actuals_has_no_mape():
    metrics = sf.error_metrics([0, 0], [1, 1])
